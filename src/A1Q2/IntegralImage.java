...
public class IntegralImage {
	...
	public IntegralImage(int[][] image) throws InvalidImageException {
		...
		// values in integer
		for (int i = 0; i < imageWidth; i++) { // check for the image. return
												// error if not rectangle
			...
			for (int j = 0; j < imageHeight; j++) { // constructing the image if
													// it passes the
	...
	public double meanSubImage(int top, int bottom, int left, int right)
		...
		if (bottom < 0 || left < 0 || top < 0 || right < 0) // loop to check for
															// boundaries
		{
			throw new BoundaryViolationException();
		} else // if no violations are done then calculate the mean of the sub
				// image values
		{
			int tLess = top - 1;
			int lLess = left - 1;
